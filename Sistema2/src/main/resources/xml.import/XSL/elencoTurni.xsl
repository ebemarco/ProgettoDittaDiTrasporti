...
<!--                <h5><xsl:value-of select="//d:autisti.xsd"/></h5>-->
    ...
    <xsl:template match="tur:turno">
            ...
            <tr>
                ...
                <td>
                    <!-- TODO: ricerca turno in base al codice. PRESTA ATTENZIONE AI NAMESPACE !!! -->
                    <xsl:variable name="codiceAutista" select="tur:turno/@codice"/>
<!--                    <xsl:value-of select="tur:turno/@codice"/>-->
<!--                    <xsl:text> </xsl:text>-->
<!--                    <xsl:value-of select="tur:turno/@codice"/>-->
<!--                    <xsl:value-of select="//d:turno[@codice='A001']"/>-->
                    <xsl:value-of select="//d:turno[@codice=$codiceAutista]/common:nome"/>
                    <xsl:text> </xsl:text>
                    <xsl:value-of select="//d:turno[@codice=$codiceAutista]/common:cognome"/>
                </td>
                <td>
